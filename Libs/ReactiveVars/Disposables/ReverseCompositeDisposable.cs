...
/// </summary>
public sealed class ReverseCompositeDisposable : ICollection<IDisposable>, ICancelable
{
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="ReverseCompositeDisposable"/> class with no disposables contained by it initially.
	/// </summary>
	public ReverseCompositeDisposable()
	{
		_disposables = [];
	}

	/// <summary>
	/// Initializes a new instance of the <see cref="ReverseCompositeDisposable"/> class with the specified number of disposables.
	/// </summary>
	/// <param name="capacity">The number of disposables that the new ReverseCompositeDisposable can initially store.</param>
	/// <exception cref="ArgumentOutOfRangeException"><paramref name="capacity"/> is less than zero.</exception>
	public ReverseCompositeDisposable(int capacity)
	{
		if (capacity < 0)
		{
	...
	/// <summary>
	/// Initializes a new instance of the <see cref="ReverseCompositeDisposable"/> class from a group of disposables.
	/// </summary>
	...
	public ReverseCompositeDisposable(params IDisposable[] disposables)
	{
		if (disposables == null)
		{
	...
	/// <exception cref="ArgumentNullException"><paramref name="disposables"/> is <c>null</c>.</exception>
	/// <exception cref="ArgumentException">Any of the disposables in the <paramref name="disposables"/> collection is <c>null</c>.</exception>
	public ReverseCompositeDisposable(IEnumerable<IDisposable> disposables)
	{
		if (disposables == null)
		{
			throw new ArgumentNullException(nameof(disposables));
	...
	/// <summary>
	/// Gets the number of disposables contained in the <see cref="ReverseCompositeDisposable"/>.
	/// </summary>
	public int Count => Volatile.Read(ref _count);

	/// <summary>
	/// Adds a disposable to the <see cref="ReverseCompositeDisposable"/> or disposes the disposable if the <see cref="ReverseCompositeDisposable"/> is disposed.
	/// </summary>
	...
	/// <summary>
	/// Removes and disposes the first occurrence of a disposable from the <see cref="ReverseCompositeDisposable"/>.
	/// </summary>
	...
	public void Dispose()
	{
		...
		{
			foreach (var d in currentDisposables.Rev())
			{
				d?.Dispose();
			}
		}
	...
	/// <summary>
	/// Removes and disposes all disposables from the <see cref="ReverseCompositeDisposable"/>, but does not dispose the <see cref="ReverseCompositeDisposable"/>.
	/// </summary>
	public void Clear()
	{
		...
		foreach (var d in previousDisposables.Rev())
		{
			d?.Dispose();
		}
	}
	...
	/// <summary>
	/// Determines whether the <see cref="ReverseCompositeDisposable"/> contains a specific disposable.
	/// </summary>
	...
	/// <summary>
	/// Copies the disposables contained in the <see cref="ReverseCompositeDisposable"/> to an array, starting at a particular array index.
	/// </summary>
	...
	/// <summary>
	/// Returns an enumerator that iterates through the <see cref="ReverseCompositeDisposable"/>.
	/// </summary>
	...
	private sealed class CompositeEnumerator : IEnumerator<IDisposable>
	{
		...
		public void Dispose()
		{
			...
			System.Array.Clear(disposables, 0, disposables.Length);
		}
